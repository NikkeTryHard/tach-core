...
fn expr_to_pytest_id(expr: &ast::Expr) -> Option<String> {
    match expr {
        ...
        ast::Expr::Name(n) => Some(n.id.to_string()),
        // Attribute expressions (like Enum values: Color.RED, Status.ACTIVE)
...
/// Expand a test function into multiple TestCases if it has @pytest.mark.parametrize decorators.
///
/// Returns a vector of TestCases - either expanded parametrized variants or a single base test.
/// This is the single source of truth for parametrize expansion, eliminating duplication.
...
fn expand_parametrized_tests(
    ...
    is_async: bool,
    line_number: usize,
    parametrized_args: Vec<String>,
    timeout_secs: Option<u64>,
    markers: Vec<String>,
    marker_info: Vec<MarkerInfo>,
) -> Vec<TestCase> {
    // Try to extract parametrize info - returns None if values are dynamic
    let param_infos = match extract_all_parametrize_info(decorators) {
        Some(infos) if !infos.is_empty() => infos,
        _ => {
            // No parametrize decorators or dynamic values - return base test only
            return vec![TestCase {
                name: name.to_string(),
                dependencies,
                is_async,
...
fn analyze_function(
    ...
    if name.starts_with("test_") {
        ...
        // Expand parametrized tests using shared helper
        let expanded = expand_parametrized_tests(
            name,
            &func.decorator_list,
            dependencies,
            is_async,
