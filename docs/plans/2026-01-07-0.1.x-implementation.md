...
#### Seccompiler Evaluation (Completed 2026-01-07)

**Status**: Already adopted and in use.

The project already uses `seccompiler` v0.5.0 (latest) from rust-vmm/seccompiler. This crate provides the high-level seccomp-bpf API used by Firecracker.

**Current usage** (`src/isolation/sandbox.rs`):

- `SeccompFilter::new()` for building syscall blacklists
- `BpfProgram` compilation via `try_into()`
- `apply_filter()` for installing the filter

**Benefits realized**:

- No raw BPF bytecode manipulation needed
- Architecture-aware (x86_64/aarch64) filter compilation
- Clean API for blacklist-based filtering
- EPERM error action instead of SIGSYS for graceful Python error handling

**No action needed** - the crate is already integrated and provides all required functionality.

### Python Compatibility Matrix

| Python | Status           | Notes                 |
| ------ | ---------------- | --------------------- |
| 3.10   | Supported (MSRV) | Minimum supported     |
| 3.11   | Supported        |                       |
