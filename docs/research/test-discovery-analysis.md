...
>
> **Note:** Test counts in this document are approximate and may change as the codebase evolves.
> The categories and patterns described remain stable even as specific counts vary.
...
1. **Ignored tests** across the codebase, organized by category
2. **Categories** of ignored tests (Environment, Slow, WIP/Experimental)
...
_Most ignored tests fall in this category._
...
| ----------------------- | ---------------------------------------------------------------------------------- |
| **Environment**         | Requires built binary, sudo, Python, or specific setup (majority of ignored tests) |
| **Slow**                | Long-running performance/stress tests (1000+ iterations)                           |
| **WIP/Experimental**    | Proof-of-concept or exploration tests                                              |
| **Kernel Requirements** | No tests ignored solely for kernel requirements (handled via graceful degradation) |
| **Flaky**               | No tests marked as flaky                                                           |

---

## 3. Category Details

### 3.1 Environment-Dependent Tests

These tests require specific environment conditions:

**Built Binary Requirement (tests in implementation_tests.rs):**
...
| File                                  | Purpose                        | Has Ignored Tests |
| ------------------------------------- | ------------------------------ | ----------------- |
| `rust_tests/implementation_tests.rs`  | Binary CLI tests               | Yes (most)        |
| `rust_tests/memory_invariant.rs`      | Memory stability tests         | Yes               |
| `rust_tests/discovery_integration.rs` | Discovery API tests            | No                |
| `rust_tests/proptest_discovery.rs`    | Property-based discovery tests | No                |
| `rust_tests/sandbox_enforcement.rs`   | Kernel sandbox tests           | No                |
| `rust_tests/kernel_compat_tests.rs`   | Graceful degradation tests     | No                |
| `src/isolation/calibration.rs`        | TLS calibration unit tests     | Yes               |
| `src/execution/plugin_bridge.rs`      | FD teleportation tests         | Yes               |
| `experiments/tls_*.rs`                | TLS exploration experiments    | Yes               |
